...
class User_handler
{
    ...
    //     ($b=$this->users_model->select_account_where_id(intval($this->CI->session->userdata('account_id'))))!==false && 
    //     !empty($b) && !empty(end($b)))
    //         return end($b);
    //     return null;
    // }
    ...
    //     ($b=$this->media_model->select_where_id(intval($a['image_id'])))!==false &&
    ...
    public function get_user_info(){
        ...
        //     $has_finger=$this->users_model->credential_where_user_mobile_id($this->get_user_mobile_id());
    ...
    public function get_notifications(){
        if($this->get_user_account_id() && ($a=$this->notification_model->get_unread_by_user_account_id($this->get_user_account_id()))!==false)
    ...
    public function get_user_info_where_user_account($id){
        ...
        return $result;
        // ($a=$this->users_model->select_account_where_id(intval($id)))!==false && 
        // !empty($a) && !empty(end($a)) && 
        // !empty(end($a)['id']) && intval(end($a)['id'])>0 && 
        // !empty(end($a)['user_mobile_id']) && intval(end($a)['user_mobile_id'])>0 &&
        // ($b=$this->users_model->select_mobile_where_id(intval(end($a)['user_mobile_id'])))!==false &&
        // !empty($b) && !empty(end($b)) && 
        // !empty(end($b)['id']) && intval(end($b)['id'])>0 && 
        // !empty(end($b)['user_id']) && intval(end($b)['user_id'])>0 && 
        // ($c=$this->users_model->select_where_id(intval(end($b)['user_id'])))!==false &&
        // !empty($c) && !empty(end($c)) && 
        // !empty(end($c)['id']) && intval(end($c)['id'])>0){
        //     $image=$this->media_model->select_where_id(end($b)['image_id']??'');
        //     return [
