...
class Page extends MY_Controller
{
	...
	private function page_info(){
		if($this->session->has_userdata('url'))
			$this->urls=$this->session->userdata('url');
		return true;
	}
	public function url(...$arr){
		if(!empty($arr)) 
			$this->session->set_userdata('url',$arr);
		$this->index();
	}
	public function index(){
		$this->main=new Main_exploder();
		$this->category=new Category_handler();
		...
		$this->set_footer();
		$this->page_info();
		return ($this->id>0?$this->dashboard():$this->login());
	}
	private function logout(){
		$this->session->sess_destroy();
		$this->header_array=$this->body_array=$this->footer_array=[];
		$this->login();
		return true;
	}
	private function set_header(){
		$this->header_array=[
			'lang' => 'fa',
			'lat'=>$this->user->location['lt'],
			'lon'=>$this->user->location['ln'],
			'id' => $this->id,
			'user_info'=>$this->user->infos,
			'chat'=>false,
			'has_auth_info_error'=>$this->user->has_code_mely_info,
			'title'=>'',
			'icon'=>'',
			'css'=>[],
    		'category'=>$this->category->valex_show(),
		];
		return true;
	}
	private function set_footer(){
		$this->footer_array=[
			'my_company'=>$this->user->company,
			'lang'=>'fa',
			'change_lang'=>'true',
			'id'=>$this->id,
			'map'=>true,
	...
	private function product(){
    	...
    	$this->body_array=array_merge($this->body_array,[
			'login'=>true,
	...
	private function template(){
		die($this->load->view('header',$this->header_array,true).
    	$this->load->view($this->body_page,$this->body_array,true).
        $this->load->view('footer',$this->footer_array,true));
	}
	private function login(){
		die($this->load->view('login',[],true));
	}
	private function dashboard(){
		if(!empty($this->urls) && !empty($this->urls['0']))
			if($this->urls['0']=='logout'){
				...
				$this->footer_array=array_merge($this->footer_array,$this->handler->footer_data);
				$this->body_page=$this->handler->page;
				$this->template();
				return true;
			}
		$this->product();
		...
		// log_message('info', '[My_logger] ' . $message);
